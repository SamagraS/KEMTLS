"""End-to-end KEMTLS OIDC demonstration using the real auth/resource servers."""
...
def load_config():
    ...
    client_cfg_path = Path(__file__).parent.parent / 'config' / 'client_config.json'
    
    if not all(os.path.exists(p) for p in [ca_path, as_path, rs_path, pdk_path, client_cfg_path]):
    ...
    with open(client_cfg_path) as f: client_cfg = json.load(f)
    ...
    return {
        ...
        'scope': client_cfg.get('scope', 'openid profile email'),
    }
...
def run_demo(mode="auto"):
    ...
    auth_url = client.start_auth(scope=config['scope'])
    ...
    print(f"[AUTH] Calling real /authorize for: {params.get('client_id')}")
    auth_resp = http_client.get(auth_url)
    if auth_resp.get('status') != 200:
        raise RuntimeError(f"Authorize failed: {auth_resp.get('body')}")
    code = auth_resp.get('body', {}).get('code')
    ...
    resource_resp = client.call_api(f"{config['rs_url']}/userinfo")
    ...
    replay_resp = client.replay_attack(f"{config['rs_url']}/userinfo")
    
    if replay_resp.get('status') in (401, 403):
        print(f"[SUCCESS] Replay attack blocked (status {replay_resp.get('status')}).")
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n[ERROR] Demo failed: {e}")
        print("Note: Ensure run_kemtls_auth_server.py and run_kemtls_resource_server.py are running.")
